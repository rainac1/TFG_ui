...
import utils.trackutil as trackutil
from configs.trackconfig import parse_args
# from datasets.generate_dataset import GeneratorDataset
from datasets.image_dataset import ImagesDataset
# from utils.face_detector import FaceDetector
from tracklib.flame.FLAME import FLAME, FLAMETex
from utils.image import tensor2im
from tracklib.renderer import Renderer
...
mediapipe_idx = np.load('tracklib/flame/mediapipe/mediapipe_landmark_embedding.npz', allow_pickle=True, encoding='latin1')['landmark_indices'].astype(int)
...
def angle2mat(angle):
    if use_6d:
        return rotation_6d_to_matrix(angle)
    shape = angle.shape
    angle = angle.contiguous()
    sinx, siny, sinz = torch.chunk(torch.sin(angle), 3, dim=-1)
    cosx, cosy, cosz = torch.chunk(torch.cos(angle), 3, dim=-1)
    ones = torch.ones_like(sinx)
    zeros = torch.zeros_like(sinx)
    rotx = torch.cat([ones, zeros, zeros,
                      zeros, cosx, -sinx,
                      zeros, sinx, cosx], dim=-1).reshape(*shape[:-1], 3, 3)
    roty = torch.cat([cosy, zeros, siny,
                      zeros, ones, zeros,
                      -siny, zeros, cosy], dim=-1).reshape(*shape[:-1], 3, 3)
    rotz = torch.cat([cosz, -sinz, zeros,
                      sinz, cosz, zeros,
                      zeros, zeros, ones], dim=-1).reshape(*shape[:-1], 3, 3)
    mat = torch.matmul(rotz, torch.matmul(roty, rotx))
    return mat
...
class Tracker(object):
    def __init__(self, config, device='cuda:0'):
        ...
        self.output_folder = self.config.workspace
        self.checkpoint_folder = os.path.join(self.save_folder, "tracker_ckpt")
        # self.input_folder = os.path.join(self.save_folder, "input")
        self.pyramid_folder = os.path.join(self.save_folder, "pyramid")
        # self.mesh_folder = os.path.join(self.output_folder, "mesh")
    ...
    def setup_renderer(self):
        mesh_file = '../../data/FLAME2020/head_template_mesh.obj'
    ...
    def save_checkpoint(self, frame_id):
        ...
        frame = {
            'flame': {
                ...
                'l_eyelid': f'{os.path.abspath(os.path.dirname(__file__))}/tracklib/flame/blendshapes/l_eyelid.npy',
                'r_eyelid': f'{os.path.abspath(os.path.dirname(__file__))}/tracklib/flame/blendshapes/r_eyelid.npy',
            ...
            'img_size': self.image_size.clone().detach().cpu().numpy().flatten(),
            'frame_id': frame_id,
            'global_step': self.global_step
        }

        vertices, lmk68, _ = self.flame(
        ...
        frame['lmk68'] = lmk68[0].cpu().numpy()
        frame['bound'] = np.stack([vertices[0].cpu().numpy().min(0), vertices[0].cpu().numpy().max(0)])
    ...
    def create_parameters(self):
        ...
        self.R = nn.Parameter(mat2angle(R).to(self.device))
    ...
    def optimize_camera(self, batch, steps=1000):
        ...
        for k in t:
            ...
            losses['lmk68'] = trackutil.lmk_loss(points68, landmarks[..., :2], [h, w], lmk_mask) * self.config.w_lmks
            losses['lmkMP'] = trackutil.lmk_loss(pointsMP, landmarks_dense[..., :2], [h, w], lmk_dense_mask) * self.config.w_lmks
    ...
    def optimize_color(self, batch, pyramid, params_func, pho_weight_func, reg_from_prev=False):
        ...
        aspect_ratio = trackutil.get_aspect_ratio(images)
        h, w = images.shape[2:4]
        logs = []
        ...
        for k, level in enumerate(pyramid):
            ...
            for p in range(iters):
                ...
                losses['loss/lmk_oval'] = trackutil.oval_lmk_loss(proj_lmks68, image_lmks68, image_size, lmk_mask) * self.config.w_lmks_oval
                losses['loss/lmk_68'] = trackutil.lmk_loss(proj_lmks68, image_lmks68, image_size, lmk_mask) * self.config.w_lmks_68
                losses['loss/lmk_MP'] = trackutil.face_lmk_loss(proj_lmksMP, image_lmksMP, image_size, True, lmk_dense_mask) * self.config.w_lmks
                losses['loss/lmk_eye'] = trackutil.eye_closure_lmk_loss(proj_lmksMP, image_lmksMP, image_size, lmk_dense_mask) * self.config.w_lmks_lid
                losses['loss/lmk_mouth'] = trackutil.mouth_lmk_loss(proj_lmksMP, image_lmksMP, image_size, True, lmk_dense_mask) * self.config.w_lmks_mouth
                losses['loss/lmk_iris_left'] = trackutil.lmk_loss(proj_vertices[:, left_iris_flame, ...], left_iris, image_size, mask_left_iris) * self.config.w_lmks_iris
                losses['loss/lmk_iris_right'] = trackutil.lmk_loss(proj_vertices[:, right_iris_flame, ...], right_iris, image_size, mask_right_iris) * self.config.w_lmks_iris
                ...
                if k > 0 or self.is_initializing:
                    ...
                    losses['loss/pho'] = trackutil.pixel_loss(ops['images'], sampled_image, self.parse_mask(ops, batch)) * pho_weight_func(k)
    ...
    def checkpoint(self, batch, visualizations=None, frame_dst='/video', save=True, dump_directly=False):
        if visualizations is None:
            visualizations = [[View.GROUND_TRUTH, View.LANDMARKS, View.HEATMAP],
                              [View.COLOR_OVERLAY, View.SHAPE_OVERLAY, View.SHAPE]]
        batch = self.to_cuda(batch)
        images, landmarks, landmarks_dense, _, _ = self.parse_batch(batch)

        # input_image = trackutil.to_image(batch['image'].clone()[0].cpu().numpy())

        savefolder = self.save_folder + frame_dst
        ...
        with torch.no_grad():
            ...
            for views in visualizations:
                ...
                for view in views:
                    ...
                    if view == View.LANDMARKS:
                        ...
                        gt_lmks = trackutil.tensor_vis_landmarks(gt_lmks, torch.cat([landmarks_dense, landmarks[:, :17, :]], dim=1), color='g')
                        gt_lmks = trackutil.tensor_vis_landmarks(gt_lmks, torch.cat([lmkMP, lmk68[:, :17, :]], dim=1), color='r')
            ...
            # VIDEO
            final_views = trackutil.merge_views(final_views)
            frame_id = str(self.frame).zfill(4)
            ...
            # DEPTH
            faces = torch.cat([trackutil.get_flame_extra_faces(), self.diff_renderer.faces], dim=1)
            depth_view = self.diff_renderer.render_depth(vertices, cameras=self.cameras, faces=faces)
    ...
    def output_video(self):
        trackutil.images_to_video(self.save_folder, self.config.fps)
    ...
    def initialize_tracking(self):
        ...
        for i, j in enumerate(keyframes):
            ...
            h, w = images.shape[2:4]
            pyramid_size = np.array([h, w])
            pyramid = trackutil.get_gaussian_pyramid([(pyramid_size * size, trackutil.round_up_to_odd(steps * 2)) for size, steps in self.pyr_levels], images, self.kernel_size, self.sigma)
    ...
    def write_transform_json(self):
        train_frame_array = []
        test_frame_array = []
        val_frame_array = []
        filenames = sorted(glob(f'{self.checkpoint_folder}/*.frame'))
        splits = (len(filenames) // 16 * 14, len(filenames) // 16 * 15)
        h, w = self.image_size.clone().detach().cpu().numpy().flatten().tolist()
        intrinsic = None
        lmk0 = None
        base_P = None
        bound_min = np.array([np.inf, np.inf, np.inf])
        bound_max = np.array([-np.inf, -np.inf, -np.inf])
        for idx, filename in tqdm(enumerate(filenames), total=len(filenames)):
            frame = torch.load(filename)
            P = np.eye(4)
            P[:3, :3] = frame['opencv']['R'].reshape(3, 3)
            P[:3, 3] = frame['opencv']['t'].reshape(3)
            frame_dict = {
                "transform_matrix": P.tolist(),
                "file_path": f'frame_imgs/{idx:04d}.png',
                "landmarks": frame["lmk68"].T.tolist()
            }
            if idx < splits[0]:
                train_frame_array.append(frame_dict)
            elif idx < splits[1]:
                test_frame_array.append(frame_dict)
            else:
                val_frame_array.append(frame_dict)

            if intrinsic is None:
                K = frame['opencv']['K'].reshape(3, 3)
                intrinsic = {
                    "cx": K[0, 2].item(),
                    "cy": K[1, 2].item(),
                    "fl_x": K[0, 0].item(),
            ...
            if base_P is None or lmk0 is None:
                base_P = frame_dict['transform_matrix']
                lmk0 = frame_dict['landmarks']

            bound_min = np.minimum(frame['bound'][0], bound_min)
            bound_max = np.maximum(frame['bound'][1], bound_max)

        bound = np.stack([bound_min, bound_max]).tolist()
        with open(os.path.join(self.output_folder, 'transforms_train.json'), 'w') as f:
            json.dump({"h": h, "w": w, **intrinsic, "base_transform_matrix": base_P,
                       "base_landmark": lmk0, "bound": bound, "frames": train_frame_array}, f, indent=4)
        with open(os.path.join(self.output_folder, 'transforms_test.json'), 'w') as f:
            json.dump({"h": h, "w": w, **intrinsic, "base_transform_matrix": base_P,
                       "base_landmark": lmk0, "bound": bound, "frames": test_frame_array}, f, indent=4)
        with open(os.path.join(self.output_folder, 'transforms_val.json'), 'w') as f:
            json.dump({"h": h, "w": w, **intrinsic, "base_transform_matrix": base_P,
                       "base_landmark": lmk0, "bound": bound, "frames": val_frame_array}, f, indent=4)
