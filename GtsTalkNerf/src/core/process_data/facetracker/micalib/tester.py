...
from configs.micaconfig import cfg
...
class Tester(object):
        ...
        for quality in cache.keys():
            ...
            for actor in tqdm(cache[quality].keys()):
                ...
                for m in range(meshes.shape[0]):
                    ...
                    micautil.visualize_grid(dict, savepath, size=512)

            logger.info(f"[TESTER] Stirling dataset {quality} with {images_processed} processed!")

            # micautil.save_embedding_projection(self.embeddings_lyhm, f'{self.cfg.output_dir}/stirling_test_{best_id}/stirling_{quality}_arcface_embeds.pdf')
    ...
    def now(self, best_id):
        ...
        # micautil.save_embedding_projection(self.embeddings_lyhm, f'{self.cfg.output_dir}/now_test_{best_id}/now_arcface_embeds.pdf')
