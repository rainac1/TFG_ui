...
from tracklib.flame.mediapipe.landmarks import LIPS_LANDMARK_IDS, get_idx, NOSE_LANDMARK_IDS
...
def images_to_video(path, fps=25, src='video', video_format='DIVX'):
    ...
    if len(img_array) > 0:
        out = cv2.VideoWriter(f'{path}/track_video.avi', cv2.VideoWriter_fourcc(*video_format), fps, size)
