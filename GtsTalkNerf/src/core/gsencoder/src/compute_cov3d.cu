 ...
 * changed from https://github.com/pointrix-project/msplat/blob/master/msplat/src/compute_cov3d.cu
 */
...
__device__ void compute_cov3_backward(const glm::vec3 scale,
    ...
    glm::mat3 R = unit_quaternion_to_rotmatrix(quat);
    glm::mat3 M = S * R;
    float rx = 1.f / scale.x, ry = 1.f / scale.y, rz = 1.f / scale.z;
    ...
    dL_dscale.x = - glm::dot(Mt[0], dL_dMt[0]);
    dL_dscale.y = - glm::dot(Mt[1], dL_dMt[1]);
    dL_dscale.z = - glm::dot(Mt[2], dL_dMt[2]);
