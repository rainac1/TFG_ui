...
def get_encoder(encoding, input_dim=3,
    ...
    elif encoding == 'landmark':
        from gsencoder import GsLmkEncoder
        encoder = GsLmkEncoder(degree=degree)

    elif encoding == 'audio':
        encoder = nn.Sequential(nn.Linear(input_dim, int(input_dim * degree)), nn.ReLU())
        return encoder, int(input_dim * degree)

    else:
        raise NotImplementedError(
            'Unknown encoding mode, choose from [None, frequency, spherical_harmonics, hashgrid, tiledgrid, landmark]')
